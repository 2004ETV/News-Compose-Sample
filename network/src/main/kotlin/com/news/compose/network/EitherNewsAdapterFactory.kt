package com.news.compose.network
...
internal class EitherNewsAdapterFactory @Inject constructor() : CallAdapter.Factory() {
    override fun get(
        returnType: Type,
        ...
        val errorBodyConverter = retrofit.responseBodyConverter<NewsErrorNetwork>(
            NewsErrorNetwork::class.java,
            annotations
        )
    ...
    private class ResultCallAdapter<R, T>(
        ...
        private val errorBodyConverter: Converter<ResponseBody, NewsErrorNetwork>
    ) :
    ...
    private class ResultCallWrapper<F, T>(
        ...
        private fun getReasonByCode(
            response: Response<T>,
            errorResponse: NewsErrorNetwork
        ) = when (response.code()) {
            404 -> ErrorReason.NotFound(errorResponse.message)
            else -> ErrorReason.ServerError(errorResponse.message)
        }
    }
}
